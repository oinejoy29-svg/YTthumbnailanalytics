...
function getSubscriberOverrides() {
  try {
    return JSON.parse(localStorage.getItem(SUBSCRIBER_OVERRIDES_KEY) || "{}");
  } catch {
    return {};
  }
}

function saveSubscriberOverride(date, count) {
  const overrides = getSubscriberOverrides();
  overrides[date] = Number(count);
...
function clearSubscriberOverride(date) {
  const overrides = getSubscriberOverrides();
...
function applySubscriberOverrides(rows) {
  const overrides = getSubscriberOverrides();

  return rows.map(row => {
    if (Object.prototype.hasOwnProperty.call(overrides, row.date)) {
      return {
        ...row,
        count: Number(overrides[row.date])
      };
    }
...
function detectTags(video) {
  return MEMBERS.filter(member =>
    `${video.title || ""} ${video.description || ""}`.includes(member)
  );
}
...
function latestRecord() {
  return sortedSubscribers().at(-1) || null;
}
...
function newSubscriberForDate(date) {
  ...
  return Number(rows[index].count) -
    Number(rows[index - 1].count);
}
...
function getTodayNewSubscribers() {
  ...
  if (todayRow) {
    const prev = previousRecord(today);

    if (prev) {
      return Number(todayRow.count) - Number(prev.count);
    }
  ...
  return current - Number(latest.count);
}
...
function renderSummary() {
  ...
  const todayNew = getTodayNewSubscribers();
  ...
  const diffs = rows
    .slice(1)
    .map((row, index) =>
      Number(row.count) - Number(rows[index].count)
    );
...
function buildRolling() {
  ...
  shuffled.forEach((video, index) => {
    lanes[index % 3].push(video);
  });

  lanes.forEach((arr, index) => {
    const twice = [...arr, ...arr];

    $(`lane${index}`).innerHTML = twice.map(video => `
      <img
        class="thumb-roll"
...
function durationNum(video) {
  return Number(video.durationSeconds || 0);
}
...
function renderVideos() {
  ...
  const list = DATA.videos.filter(video =>
    !tag ||
    (video.tags || detectTags(video)).includes(tag)
  );

  list.sort((a, b) => {
    if (sort === "popular") {
      return Number(b.viewCount || 0) -
    ...
    if (sort === "longest") {
      return durationNum(b) - durationNum(a);
    }
  ...
  document.querySelectorAll(".video-card").forEach(card => {
    const open = () => {
      const video = DATA.videos.find(
        item => String(item.id) === String(card.dataset.videoId)
      );
...
function openVideoDetail(video) {
  const tags = video.tags || detectTags(video);
    ...
    <div class="video-detail-layout">
      ...
      <div class="video-detail-info">
        ...
        <div class="video-detail-meta">
          <div>
            <span>投稿日</span>
            <strong>${jpDate(video.date)}</strong>
          </div>

          <div>
            <span>再生回数</span>
            <strong>${fmt(video.viewCount)}回</strong>
          </div>

          <div>
            <span>動画時間</span>
            <strong>${escapeHtml(video.duration || "—")}</strong>
          </div>
        </div>
  ...
  $("videoDetailModal").classList.add("open");
  $("videoDetailModal").setAttribute("aria-hidden", "false");
...
function closeVideoDetail() {
  $("videoDetailModal").classList.remove("open");
  $("videoDetailModal").setAttribute("aria-hidden", "true");
}
...
function editVideoTags(video) {
  ...
  const tags = input
    ...
    .map(tag => tag.trim())
    .filter(Boolean)
    .filter(tag => MEMBERS.includes(tag));
...
function fixedNewSubscriberScale() {
  ...
  return {
    min:
      min >= 0
        ? 0
        : Math.floor((min - pad) / 5) * 5,

    max:
      Math.max(
        5,
        Math.ceil((max + pad) / 5) * 5
      )
  };
}
      ...
      tooltip: {
        callbacks: {
          ...
          afterBody: items => {
            ...
            const date =
              sortedSubscribers()[items[0].dataIndex]?.date;

            const titles =
              chartVideoTitles(date);
...
function renderCharts() {
  ...
  const subScale =
    fixedSubscriberScale(allRows);

  const newScale =
    fixedNewSubscriberScale();

  const labels =
    rows.map(row => monthDay(row.date));

  const videoDates =
    rows.map(row =>
      chartVideoTitles(row.date).length
    );

  const pointColors =
    videoDates.map(hasVideo =>
      hasVideo ? "#FF3030" : "#FFF36A"
    );
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            title: items =>
              items[0]
                ? monthDay(rows[items[0].dataIndex].date)
                : "",
  ...
  const newVals =
    rows.map(row =>
      newSubscriberForDate(row.date)
    );

  const periodTotal =
    newVals.reduce((a, b) => a + b, 0);

  const periodAverage =
    newVals.length
      ? periodTotal / newVals.length
      : 0;
...
function renderSubscriberHistory() {
  ...
  const visibleRows =
    rows.slice(
      Math.max(
        0,
        rows.length - subscriberHistoryLimit
      )
    ).reverse();

  $("subscriberHistoryBody").innerHTML =
    visibleRows.map(row => {
      const newCount =
        newSubscriberForDate(row.date);
      ...
      return `
        <tr>
          <td>${jpDate(row.date)}</td>

          <td class="history-count">
            ${fmt(row.count)}人
          </td>

          <td class="history-new ${
            newCount < 0 ? "negative" : ""
          }">
  ...
  document
    .querySelectorAll(".history-edit-btn")
    .forEach(button => {
      button.onclick = () => {
...
function editSubscriberRecord(date) {
  const row =
  ...
  if (!Number.isFinite(count) || count < 0) {
    alert("正しい登録者数を入力してください。");
    return;
  }

  saveSubscriberOverride(date, Math.round(count));
...
function getWindowStats(n, endIndex = null) {
  ...
  const startCount =
    startIndex > 0
      ? Number(rows[startIndex - 1].count)
      : Number(arr[0].count);

  const total =
    Number(arr.at(-1).count) -
    startCount;
...
function buildCollagePages() {
  const pages =
    Math.max(
      1,
      Math.ceil(DATA.videos.length / 100)
    );

  $("collagePages").innerHTML =
    Array.from(
      { length: pages },
      (_, index) => `
        <button
  ...
  document
    ...
    .forEach(button => {
      button.classList.toggle(
        "active",
        Number(button.dataset.page) === page
      );
    });
...
async function downloadCollage() {
  const page =
    Number(
      $("downloadCollage").dataset.page || 0
    );
...
function setupRange() {
  document
    .querySelectorAll(".preset-group button")
    .forEach(button => {
      button.onclick = () => {
        const value =
...
function setupModals() {
  $("closeVideoDetail").onclick =
    closeVideoDetail;

  $("videoDetailModal").onclick =
    event => {
      if (
        event.target.id ===
        "videoDetailModal"
  ...
  $("closeCollage").onclick =
    closeCollage;

  $("collageModal").onclick =
    event => {
      if (
        event.target.id ===
        "collageModal"
